...
{
    public class AutoClusterization : IClusterization
    {
        public List<Sensor> Clusterize()
        {
            ...
            int clustersCount = (int)Math.Floor(sensors.Count * 0.05);
            if (clustersCount % 2 == 1)
            {
                clustersCount++;
            }
            ...
            int countX = (int)Math.Floor(Math.Sqrt(clustersCount));
            int countZ = (int)Math.Ceiling(Math.Sqrt(clustersCount));
            ...
            double clusterLength = areaLength / countX + 1;
            double clusterWidth = areaWidth / countZ + 1;
